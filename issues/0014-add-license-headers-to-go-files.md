...
Medium。`LICENSE` ファイルがリポジトリ直下に存在し Apache 2.0 であることは明示されているため、ライセンスの法的効力は確保されている。ただし各ソースファイルへのヘッダー付与は Apache 2.0 付録の推奨慣行であり、ソースコードが単体で流通した場合のライセンス明示のために実施する。18 ファイルの機械的な追加であり作業負荷は低い。
